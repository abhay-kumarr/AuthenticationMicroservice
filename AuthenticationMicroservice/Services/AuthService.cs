...
namespace AuthenticationMicroservice.Services
{
    ...
    {
        private const string SECRET_KEY = "secret-key-value--AHGTYytr7@hgf$GHF";
        ...
        public bool IsUserValid(User user, out AuthUser authUser)
        {
            ...
            {
                authUser = null;
                string reqAddress = string.Empty;
                switch (user.Role)
                {
                    case Role.Manager:
                        reqAddress = API.ManagerMicroservice.ValidateManagerUrl();
                        break;
                    case Role.Executive:
                        reqAddress = API.ManagerMicroservice.ValidateExecutiveUrl();
                        break;
                    case Role.Customer:
                        reqAddress = API.CustomerMicroservice.ValidateUrl();
                        break;
                    default:
                        break;
                }
